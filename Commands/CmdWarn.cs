    ...
    public class CmdWarn : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (p == null)
            {
                ...
                {
                    Player.SendMessage(p, "why are you warning a dev??"); return;
                }
                Player.GlobalMessage("console %ewarned " + who.color + who.name + " %ebecause : &c");
                Player.GlobalMessage(reason);
                Player.SendMessage(who, "Do it again "); return;
                {
                    if (who.warn == 0)
                    {
                    ...
                    if (who.warn == 2)
                    {
                        ...
                        Player.GlobalMessage(reason);
                        who.warn = 0;
                        who.Kick("BEACUASE " + reason + ""); return;
                    }
                }
            }
            ...
            {
                Player.SendMessage(p, "you can't warn a player eqaul or higher rank."); return;
            }
            if (Server.devs.Contains(p.name))
            {
                Player.SendMessage(p, "why are you warning a dev??"); return;
            ...
            if (who == p)
            {
                Player.SendMessage(p, "you can't warn yourself"); return;
            }
            {
        ...
        {
            Player.SendMessage(p, "/warn - Warns a player."); return;
        }
    }
}